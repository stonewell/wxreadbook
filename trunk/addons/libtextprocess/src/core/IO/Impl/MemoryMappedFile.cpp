...
wxChar * TextProcess::IO::Impl::CMemoryMappedFile::DecodeData(wxFileOffset nOffset, wxFileOffset nLength, wxInt32 & nDecodedSize)
{
	...
	if (nDecodedSize == wxCONV_FAILED || 
		(nDecodedSize == 1 && nLength > 0))
	{
		nDecodedSize = (nLength + sizeof(wxChar) - 1) / sizeof(wxChar);

		wxChar * pBuf = m_StringPool.AllocReadOnlyString(nDecodedSize + 1);
		memmove(pBuf, pBegin, nLength);
		pBuf[nDecodedSize] = wxT('\0');
